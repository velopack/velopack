...
and boots the CI environment in the background while you work:
    ...
    blacksmith testbox warmup testbox.yml
    # → tbx_01jkz5b3t9...
...
1. Warm up immediately when you receive a coding task:
   `blacksmith testbox warmup testbox.yml` → save the ID
...
If you need a longer session, increase the timeout at warmup time:
    ...
    blacksmith testbox warmup testbox.yml --idle-timeout 60

## With options

    blacksmith testbox warmup testbox.yml --ref main
    blacksmith testbox warmup testbox.yml --idle-timeout 60
