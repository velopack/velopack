...
- Rust→Python logging uses the in-repo bridge `src/lib-python/src/logger.rs`, NOT the `pyo3-log` crate (it pins `pyo3 < 0.29` and was holding back pyo3 upgrades).
- pyo3 is held at 0.28 with RUSTSEC-2026-0176/0177 ignored in `deny.toml` because pyo3-stub-gen does not compile against pyo3 0.29 yet. When pyo3-stub-gen catches up: bump `pyo3`/`pyo3-build-config` to 0.29, bump `pyo3-stub-gen`, change `pyo3/abi3-py37` → `abi3-py38` in lib-python's `extension` feature (0.29 dropped py37; pyproject already requires `>=3.8`), and remove the two deny.toml ignores.
