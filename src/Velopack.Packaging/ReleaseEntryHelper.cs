    ...
    public class ReleaseEntryHelper
    {
        ...
        public void ValidateChannelForPackaging(SemanticVersion version, string channel, RID rid)
        {
            ...
            foreach (var release in _releases[channel]) {
                if (release.Rid != rid) {
                    throw new UserInfoException("All releases in a channel must have the same RID. Please correct RELEASES file or change channel name: " + GetReleasePath(channel));
                }
                if (version <= release.Version) {
                    throw new UserInfoException($"Release {release.OriginalFilename} is equal or newer to the current version {version}. Please increase the current package version or remove that release.");
                }
            }
        }
