...
fn run() -> Result<()> {
    ...
    // Common paths
    let exe_path = r"C:\Source\velopack\target\debug\testapp.exe";
    let working_dir = r"C:\Source\velopack\target\debug";
    ...
    if args.iter().any(|arg| arg == step1_arg) {
        log_message("About to run step 2 (launch self FROM elevated process)...")?;
        velopack::process::run_process(exe_path, vec![step2_arg.into()], Some(working_dir), true, Some(set_env))?;
