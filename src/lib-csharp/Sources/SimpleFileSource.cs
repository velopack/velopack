...
namespace Velopack.Sources
{
    ...
    public class SimpleFileSource : IUpdateSource
    {
        ...
        /// <inheritdoc />
        public virtual async Task<VelopackAssetFeed> GetReleaseFeed(ILogger logger, string channel, Guid? stagingId = null,
            VelopackAsset? latestLocalRelease = null)
        {
            ...
            if (File.Exists(feedLoc)) {
                ...
                return VelopackAssetFeed.FromJson(await IoUtil.ReadAllTextAsync(feedLoc).ConfigureAwait(false));
            }
            ...
            foreach (var pkg in Directory.EnumerateFiles(BaseDirectory.FullName, "*.nupkg")) {
                try {
                    var manifest = await ZipPackage.ReadManifestAsync(pkg).ConfigureAwait(false);
                    var asset = await VelopackAsset.FromManifest(pkg, manifest, true).ConfigureAwait(false);
                    if (asset?.Version != null) {
                        if (channel == null || manifest?.Channel == null || manifest?.Channel == channel) {
                            logger.Debug($"Read package '{pkg}' with version '{asset.Version}' in channel '{manifest?.Channel}'.");
                            list.Add(asset);
                        } else {
