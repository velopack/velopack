...
public class FileSourceTests(DockerFixture fixture, ITestOutputHelper output)
{
    ...
    private static void AssertTarget(UpdateInfo info)
    {
        ...
        Assert.Equal(TestFeedData.NotesMarkdown(TestFeedData.LatestVersion), info.TargetFullRelease.NotesMarkdown);
        Assert.Equal(TestFeedData.NotesHTML(TestFeedData.LatestVersion), info.TargetFullRelease.NotesHTML);
    }
    ...
    private static void AssertFeed(VelopackAssetFeed feed)
    {
        ...
        var fullWithNotes = feed.Assets.First(a => a.Type == VelopackAssetType.Full);
        Assert.NotNull(fullWithNotes.NotesMarkdown);
        Assert.NotEmpty(fullWithNotes.NotesMarkdown);
        Assert.NotNull(fullWithNotes.NotesHTML);
        Assert.NotEmpty(fullWithNotes.NotesHTML);
    }
    ...
    private static void AssertHarnessNotesHtml(HarnessResult result)
    {
        ...
        Assert.NotNull(result.Target.NotesMarkdown);
        Assert.NotEmpty(result.Target.NotesMarkdown);
        Assert.Contains("## Release", result.Target.NotesMarkdown);
    }
